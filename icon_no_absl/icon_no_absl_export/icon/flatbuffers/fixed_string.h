#ifndef ICON_FLATBUFFERS_FIXED_STRING_H_
#define ICON_FLATBUFFERS_FIXED_STRING_H_
...
// Returns an InvalidArgumentError if `destination` is too small to hold the
// entirety of `source`.
...
void StringCopy(T* destination, std::string_view source) {
  ...
  size_t copy_count = std::min(static_cast<size_t>(MaxSize), source.size());

  std::memcpy(destination->mutable_data()->Data(), source.data(), copy_count);
  destination->mutate_size(copy_count);
}
...
std::string_view StringView(const T* source) {
  ...
  return std::string_view(reinterpret_cast<const char*>(source->data()->data()),
                          source->size());
}

}  // namespace intrinsic_fbs
#endif  // ICON_FLATBUFFERS_FIXED_STRING_H_
