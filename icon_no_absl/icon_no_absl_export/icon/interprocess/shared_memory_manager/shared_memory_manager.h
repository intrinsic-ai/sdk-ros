...
class SharedMemoryManager final {
 public:
  ...
  const ::intrinsic::icon::SegmentNameToFileDescriptorMap&
  SegmentNameToFileDescriptorMap() const {
  ...
  // `payload_size` bytes.
  Status AddSegment(std::string_view name, bool must_be_used,
                    size_t payload_size) {
  ...
  Status AddSegment(std::string_view name, bool must_be_used,
    ...
    return InitSegment(name, must_be_used, payload_size, type_id);
  }
 ...
 private:
  ...
  // POSIX calls fail.
  Status InitSegment(std::string_view name, bool must_be_used,
                     size_t payload_size, const std::string& type_id);
  ...
  ::intrinsic::icon::SegmentNameToFileDescriptorMap
...
#endif  // ICON_INTERPROCESS_SHARED_MEMORY_MANAGER_SHARED_MEMORY_MANAGER_H_
