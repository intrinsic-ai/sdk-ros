...
tl::expected<RemoteTriggerServer, Status> RemoteTriggerServer::Create(
    ...
    std::string_view server_memory_name, const log::Logger* logger,
    Callback&& callback) {
  std::string request_name =
      std::string(server_memory_name) + kSemRequestSuffix;
  std::string response_name =
      std::string(server_memory_name) + kSemResponseSuffix;

  if (auto status = shm_manager.AddSegment<BinaryFutex>(request_name,
                                                        /*must_be_used=*/false,
                                                        BinaryFutex());
      !status.ok()) {
  ...
  if (auto status = shm_manager.AddSegment<BinaryFutex>(response_name,
                                                        /*must_be_used=*/false,
                                                        BinaryFutex());
      !status.ok()) {
    return tl::make_unexpected(status);
  ...
  auto request_futex =
      shm_manager.Get<ReadOnlyMemorySegment<BinaryFutex>>(request_name, logger);
  if (!request_futex.has_value()) {
    return tl::make_unexpected(request_futex.error());
  }

  auto response_futex = shm_manager.Get<ReadWriteMemorySegment<BinaryFutex>>(
      response_name, logger);
  if (!response_futex.has_value()) {
    return tl::make_unexpected(response_futex.error());
  }
  return RemoteTriggerServer(
      server_memory_name, std::move(request_futex.value()),
      std::move(response_futex.value()), std::move(callback));
}

RemoteTriggerServer::RemoteTriggerServer(
    std::string_view server_memory_name,
    ReadOnlyMemorySegment<BinaryFutex>&& request_futex,
    ReadWriteMemorySegment<BinaryFutex>&& response_futex, Callback&& callback)
...
RemoteTriggerServer::~RemoteTriggerServer() {
  ...
  // nullptr)
  if (auto* response_futex_ptr =
          reinterpret_cast<BinaryFutex*>(response_futex_.GetRawValue());
...
Status RemoteTriggerServer::StartAsync(const log::Logger* logger,
                                       RemoteTriggerServer::Prelude prelude) {
  // Report incomplete shutdown
  if (!is_running_.load(std::memory_order_acquire) &&
      async_thread_.joinable()) {
    return Status{
        .code = StatusCode::kFailedPrecondition,
        .message =
            {"RemoteTriggerServer is not running, but its async thread is "
             "active. "
...
void RemoteTriggerServer::Run(const log::Logger* logger,
                              RemoteTriggerServer::Prelude prelude) {
  if (prelude) {
