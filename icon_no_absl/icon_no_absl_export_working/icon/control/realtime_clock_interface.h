...
class RealtimeClockInterface {
 public:
  ...
  virtual RealtimeStatus TickBlockingWithDeadline(Time current_timestamp,
                                                  Time deadline) = 0;
  ...
  virtual RealtimeStatus Reset(std::chrono::nanoseconds timeout) = 0;

  virtual ~RealtimeClockInterface() = default;
};

}  // namespace intrinsic
#endif  // ICON_CONTROL_REALTIME_CLOCK_INTERFACE_H_
