#ifndef ICON_INTERPROCESS_SHARED_MEMORY_MANAGER_DOMAIN_SOCKET_SERVER_H_
#define ICON_INTERPROCESS_SHARED_MEMORY_MANAGER_DOMAIN_SOCKET_SERVER_H_
...
class DomainSocketServer {
 public:
  static constexpr std::chrono::seconds kDefaultLockAcquireTimeout =
  ...
  // them.
  Status ServeShmDescriptors(const SegmentNameToFileDescriptorMap&
  ...
  // served.
  Status AddSegmentInfoServeShmDescriptors(
 ...
 private:
  ...
  DomainSocketServer(std::filesystem::path absolute_socket_path,
  ...
  tl::expected<std::vector<std::unique_ptr<const Message>>, Status>
  PrepareMessages(const SegmentNameToFileDescriptorMap&
                      segment_name_to_file_descriptor_map);
  ...
  std::filesystem::path absolute_socket_path_;
  // Absolute path to the lock file. Only required for logging.
  std::filesystem::path absolute_lock_path_;

  std::string module_name_;
  std::string shared_memory_namespace_;

  // TODO(nilsb): replace with std::latch, std::barrier or std::atomic_flag_wait
  //              when we can assume compiler support.
...
#endif  // ICON_INTERPROCESS_SHARED_MEMORY_MANAGER_DOMAIN_SOCKET_SERVER_H_
