...
inline long futex(
  std::atomic<uint32_t> * uaddr, int futex_op, uint32_t val,
  bool private_futex,
...
// Wait until `val` changes from kReady to something else, or until `ts_absolute`.
//
// `ts_absolute` must be an absolute timeout in CLOCK_MONOTONIC.
RealtimeStatus Wait(
  std::atomic<uint32_t> & val, const timespec * ts_absolute,
  bool private_futex)
{
  ...
  while (true) {
    ...
    // equal to kReady.
    //
    // N.B.: FUTEX_WAIT_BITSET interprets `ts_absolute` as an absolute timeout in
    // CLOCK_MONOTONIC (it would use CLOCK_REALTIME if we set FUTEX_CLOCK_REALTIME).
    auto ret = futex(&val, FUTEX_WAIT_BITSET,
                     BinaryFutex::kReady, private_futex, ts_absolute);
    ...
    if (ret == -1 && errno != EAGAIN && errno != EINTR) {
      ...
      // TODO(nilsb): Port intrinsic::StrError and then print error message instead of errno.
      (void) std::snprintf(
          status.message.data(),
          status.message.size(),
          "Futex wait failed with error: %d", errno);
      return status;
    }
  }
}
...
BinaryFutex::BinaryFutex(BinaryFutex && other)
...
uint32_t BinaryFutex::Value() const
{
  return val_.load();
}
...
void BinaryFutex::Close()
{
  ...
  if (val_.exchange(kClosed) != kClosed) {
    ...
    (void) futex(&val_, FUTEX_WAKE, INT_MAX, private_futex_);
  }
}
