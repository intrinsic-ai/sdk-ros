...
private:
  ...
  tl::expected<std::vector<std::unique_ptr<const Message>>, Status> PrepareMessages(
    const SegmentNameToFileDescriptorMap &
    segment_name_to_file_descriptor_map);
...
// LINT.ThenChange(//intrinsic/icon/interprocess/shared_memory_manager/domain_socket_utils.h:expected_version)
